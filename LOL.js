window.getLOLQuizData = function() {
  return [
    // ★ normal（基本問題）
    {
      difficulty: 'normal',
      question: '移動速度を10秒間24-48%増加させ、ユニットをすり抜けられるサモナースペルは？',
      choices: ['ゴースト', 'ヒール', 'フラッシュ', 'イグナイト'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: '自身と味方1人のHPを回復し、移動速度も一時的に上昇させるサモナースペルは？',
      choices: ['ヒール', 'バリア', 'スマイト', 'ゴースト'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: '2.5秒間、自身にシールドを付与するサモナースペルは？',
      choices: ['バリア', 'クレンズ', 'イグゾースト', 'マーク/ダッシュ'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: '対象の敵にスロー効果と与ダメージ減少を与えるサモナースペルは？',
      choices: ['イグゾースト', 'ヒール', 'フラッシュ', 'スマイト'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: '自身にかかっている多くのCCやイグゾースト効果を解除するサモナースペルは？',
      choices: ['クレンズ', 'ヒール', 'バリア', 'フラッシュ'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: '指定地点にワープできる、汎用性の高いサモナースペルは？',
      choices: ['フラッシュ', 'テレポート', 'ゴースト', 'ヒール'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: '10分経過後に使用可能になり、到着後移動速度も上昇するサモナースペルは？',
      choices: ['真テレポート', 'テレポート', 'フラッシュ', 'ゴースト'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: '対象の敵に5秒間かけてTrueDMと負傷効果を与えるサモナースペルは？',
      choices: ['イグナイト', 'イグゾースト', 'クレンズ', 'フラッシュ'],
      answer: 0
    },
    {
  difficulty: 'normal',
  question: '大型モンスターに600/1000/1400のTrueDMを与えるジャングル向けサモナースペルは？',
  choices: ['スマイト', 'マーク/ダッシュ', 'テレポート', 'クラリティ'],
  answer: 0
},
    {
      difficulty: 'normal',
      question: 'ダーク シールの効果は？',
      choices: ['敵チャンピオンキル時にスタックがたまる', 'ミニオンに追加ダメージ', 'HP回復効果', '防御力上昇'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: 'ドラン ブレードの効果は？',
      choices: ['ライフスティール効果を持つ', 'AP上昇', 'マナ回復', 'ワード設置'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: 'コントロール ワードの特徴は？',
      choices: ['破壊されるまで永続設置', '時間経過で消える', '透明化する', '味方だけ視認可能'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: 'ファーサイト オルタレーションの特徴は？',
      choices: ['遠距離からワード設置できる', 'ステルス検知', 'シールド付与', 'ミニオン即死'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: 'オラクル レンズの効果は？',
      choices: ['ステルスワードを可視化', '味方の攻撃速度増加', '自身の移動速度上昇', 'スキルクール短縮'],
      answer: 0
    },

    {
      difficulty: 'normal',
      question: 'LOLで体力を回復するサモナースペルはどれ？',
      choices: ['ヒール', 'フラッシュ', 'スマイト', 'テレポート'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: '敵に近づくためによく使われるサモナースペルは？',
      choices: ['フラッシュ', 'バリア', 'イグゾースト', 'スマイト'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: 'ジャングラーがよく持つサモナースペルは？',
      choices: ['スマイト', 'ヒール', 'クレンズ', 'バリア'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: 'LOLでチャンピオンのスキルを使うために必要なものは？',
      choices: ['マナ', 'ゴールド', '経験値', '装備'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: '敵の視界を消すために使うトリンケットは？',
      choices: ['オラクルレンズ', 'ワーディングトーテム', 'ファーサイト', 'コントロールワード'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: '敵のスキルを避けるのに有効なサモナースペルは？',
      choices: ['フラッシュ', 'ヒール', 'テレポート', 'スマイト'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: '敵の位置を確認するために使うアイテムは？',
      choices: ['ワード', 'ポーション', 'リコール', 'スイープ'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: 'LOLの基本的な勝利条件は？',
      choices: ['ネクサスを破壊する', '敵を全滅させる', 'すべてのタワーを破壊する', '全員が20キルする'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: '「ADC」とはどのレーンで主に使われる？',
      choices: ['ボットレーン', 'トップレーン', 'ジャングル', 'ミッドレーン'],
      answer: 0
    },
    {
      difficulty: 'normal',
      question: 'LOLで最も重要なオブジェクトの一つは？',
      choices: ['バロンナッシャー', 'カニ', 'ミニオン', 'テレポート'],
      answer: 0
    },    
    // ★ hard（やや知識必要問題）
    {
      difficulty: 'hard',
      question: 'スマイトの進化版で、敵チャンピオンにもSlow効果を与えるバージョンは？',
      choices: ['アンリーシュドスマイト', 'プライマルスマイト', '通常スマイト', 'フラッシュスマイト'],
      answer: 0
    },
  {
  difficulty: 'hard',
  question: 'スマイトの最終強化版で、モンスターに最大1400TrueDMを与えるのは？',
  choices: ['プライマルスマイト', 'アンリーシュドスマイト', '通常スマイト', '強化スマイト'],
  answer: 0
},
    {
      difficulty: 'hard',
      question: 'スマイトを持たないと購入できないアイテムは？',
      choices: ['ジャングルアイテム', 'ファーストアイテム', 'サポートアイテム', 'スターターアイテム'],
      answer: 0
    },
    {
      difficulty: 'hard',
      question: 'イグゾーストによる与ダメ減少効果は何%？',
      choices: ['35%', '40%', '20%', '50%'],
      answer: 0
    },
    {
      difficulty: 'hard',
      question: 'ゴーストの移動速度増加効果量は何に応じて決まる？',
      choices: ['レベル', '残りHP', 'マナ量', '攻撃力'],
      answer: 0
    },
    {
      difficulty: 'hard',
      question: 'フラッシュ使用後、事前に入力していたコマンドはどうなる？',
      choices: ['キャンセルされない', 'キャンセルされる', 'リセットされる', '無効化される'],
      answer: 0
    },
    {
      difficulty: 'hard',
      question: 'クレンズ使用後、CC効果時間が何%減少する？',
      choices: ['75%', '50%', '25%', '100%'],
      answer: 0
    },
    {
      difficulty: 'hard',
      question: 'テレポートの詠唱時間は何秒？',
      choices: ['4秒', '2秒', '5秒', '3秒'],
      answer: 0
    },
    {
  difficulty: 'hard',
  question: '至点のソリの効果は？',
  choices: [
    '移動妨害時に自分と近くの味方を回復し、移動速度を増加',
    'スキル後の通常攻撃に追加物理DMと被ダメージ増加効果',
    '被ダメージを軽減し、効果終了時に周囲の敵へスロウ',
    '回復/シールド時に味方へダメージ軽減と追加魔法DM'
  ],
  answer: 0
},
{
  difficulty: 'hard',
  question: 'ブラッドソングの効果は？',
  choices: [
    '移動妨害時に自分と近くの味方を回復し、移動速度を増加',
    'スキル後の通常攻撃に追加物理DMと被ダメージ増加効果',
    '被ダメージを軽減し、効果終了時に周囲の敵へスロウ',
    '回復/シールド時に味方へダメージ軽減と追加魔法DM'
  ],
  answer: 1
},
{
  difficulty: 'hard',
  question: 'セレスティアル オポジションの効果は？',
  choices: [
    '移動妨害時に自分と近くの味方を回復し、移動速度を増加',
    'スキル後の通常攻撃に追加物理DMと被ダメージ増加効果',
    '被ダメージを軽減し、効果終了時に周囲の敵へスロウ',
    '回復/シールド時に味方へダメージ軽減と追加魔法DM'
  ],
  answer: 2
},
{
  difficulty: 'hard',
  question: 'ドリーム メーカーの効果は？',
  choices: [
    '移動妨害時に自分と近くの味方を回復し、移動速度を増加',
    'スキル後の通常攻撃に追加物理DMと被ダメージ増加効果',
    '被ダメージを軽減し、効果終了時に周囲の敵へスロウ',
    '回復/シールド時に味方へダメージ軽減と追加魔法DM'
  ],
  answer: 3
},
    
    {
      difficulty: 'hard',
      question: '「CC」とは何の略？',
      choices: ['クラウドコントロール', 'クイックキャスト', 'クリープキャッチ', 'チャージキャノン'],
      answer: 0
    },
    {
      difficulty: 'hard',
      question: '「オブジェクト志向」のプレイとは何を優先する？',
      choices: ['ドラゴンやバロンの制圧', '1v1でのキル', 'ミニオン狩り', 'アイテム収集'],
      answer: 0
    },
    {
      difficulty: 'hard',
      question: '敵ジャングルにワードを置く目的は？',
      choices: ['ガンクの察知', '敵を挑発する', '回復のため', '自分の経験値獲得'],
      answer: 0
    },
    {
      difficulty: 'hard',
      question: '視界外の敵にスキルを当てる手段は？',
      choices: ['ブッシュにワードを置く', 'スキルをばらまく', 'ミニオンに当てる', 'トーテムで攻撃'],
      answer: 0
    },
    {
      difficulty: 'hard',
      question: '敵チームがバロンを始めたか確認する方法は？',
      choices: ['バロンにワードを置く', 'マップを開く', '味方を行かせる', 'チャットで聞く'],
      answer: 0
    },
    {
      difficulty: 'hard',
      question: '「リーシュ」とは何を指す？',
      choices: ['ジャングル開始時にダメージを与えるサポート', '敵を挑発すること', 'リコールをすること', 'タワーを殴ること'],
      answer: 0
    },
    {
      difficulty: 'hard',
      question: 'バロンバフの効果に含まれるのは？',
      choices: ['ミニオン強化', 'HP回復', '経験値増加', 'スキルクール減少'],
      answer: 0
    },
    {
      difficulty: 'hard',
      question: '「ピール」とは何のこと？',
      choices: ['味方キャリーを守る動き', '敵を倒すこと', '視界を取ること', 'バロンを狩ること'],
      answer: 0
    },
    {
      difficulty: 'hard',
      question: 'ADCが持つことが少ないサモナースペルは？',
      choices: ['テレポート', 'ヒール', 'フラッシュ', 'クレンズ'],
      answer: 0
    },
    {
      difficulty: 'hard',
      question: '試合の序盤を指す用語は？',
      choices: ['レーンフェーズ', 'ミッドゲーム', 'レイトゲーム', 'ドラフト'],
      answer: 0
    },    
    // ★ veryhard（戦術・高度な応用）
    {
      difficulty: 'veryhard',
      question: '「CCチェーン」を決める際に意識すべきことは？',
      choices: ['同時にすべてのスキルを撃つ', 'CC効果が重ならないように連続で当てる', '敵の逃走経路を作る', '味方と別々の敵を狙う'],
      answer: 1
    },
    {
      difficulty: 'veryhard',
      question: 'ミニオンウェーブを「スロー・プッシュ」する利点は？',
      choices: ['敵ジャングラーを誘き出せる', '自レーンでのオールイン勝負を狙いやすい', '味方タワーを守れる', 'タワーダイブの準備ができる'],
      answer: 3
    },
    {
      difficulty: 'veryhard',
      question: 'ブッシュにワードを置く理想的なタイミングは？',
      choices: ['ガンクを受けた後', 'レーンを押し込んだ直後', 'リコール後の移動中', '敵がいないタイミング'],
      answer: 1
    },
    {
      difficulty: 'veryhard',
      question: 'オールイン（全力交戦）を仕掛けるべき最も有利なタイミングは？',
      choices: ['自分のアルティメットが上がっている時', '敵がタワー下にいる時', 'ミニオンウェーブが自陣側にいる時', '敵に視界がある時'],
      answer: 0
    },
    {
      difficulty: 'veryhard',
      question: '敵に強力なエンゲージ（開戦）手段がある場合、最も安全な立ち回りは？',
      choices: ['味方と固まらず広がる', '密集して固まる', '敵ジャングルに入る', '真ん中を歩いて進軍する'],
      answer: 0
    },
    {
      difficulty: 'veryhard',
      question: '敵ADCにCCを当てる最も効果的なタイミングは？',
      choices: ['フラッシュを使った直後', 'タワー下にいるとき', '味方が離れているとき', 'リコール中'],
      answer: 0
    },
    {
      difficulty: 'veryhard',
      question: 'CCチェーンを決めるときの注意点は？',
      choices: ['効果時間を重ねないようにする', '全員同時にCCを打つ', '敵のタワー下で使う', 'CCをばらばらに当てる'],
      answer: 0
    },
    {
      difficulty: 'veryhard',
      question: 'レーンスワップの目的として正しいのは？',
      choices: ['不利なマッチアップを回避する', '相手を怒らせる', 'キルを狙う', '森を回る'],
      answer: 0
    },
    {
      difficulty: 'veryhard',
      question: 'リバーにワードを置く最も効果的な位置は？',
      choices: ['茂みの中央', 'タワー下', '自陣の入り口', '敵のネクサス'],
      answer: 0
    },
    {
      difficulty: 'veryhard',
      question: 'ジャングラーがLv3でガンクを狙う理由は？',
      choices: ['CCとダメージが揃うため', '敵がリコールする前だから', 'ワードがないから', 'アルティメットが使えるから'],
      answer: 0
    },
    {
      difficulty: 'veryhard',
      question: 'ゾーニングとは？',
      choices: ['相手の行動範囲を制限するプレイ', 'ゾーニャを使うこと', '敵を倒すこと', '森を歩くこと'],
      answer: 0
    },
    {
      difficulty: 'veryhard',
      question: '「セーフプレイ」とはどんなスタイル？',
      choices: ['無理な仕掛けをせず安全に立ち回る', '積極的に仕掛ける', '敵のジャングルに入る', '味方と距離を取る'],
      answer: 0
    },
    {
      difficulty: 'veryhard',
      question: 'ドラゴンファイトでの最も重要な役割は？',
      choices: ['視界確保と位置取り', '敵ADCに突っ込む', 'スキルを先に使う', 'バックドアを狙う'],
      answer: 0
    },
    {
      difficulty: 'veryhard',
      question: '「オブジェクトトレード」の例として適切なのは？',
      choices: ['敵がバロン中にドラゴンを取る', '敵を倒さず帰る', '視界だけ取って下がる', '味方を見捨てる'],
      answer: 0
    },
    {
      difficulty: 'veryhard',
      question: '視界が無い場所でファイトを避けるべき理由は？',
      choices: ['敵の位置が不明で不利になりやすいから', 'ミニオンがいないから', 'リコールのタイミングだから', 'チャットで怒られるから'],
      answer: 0
    },
    
    
    
    // パッチ26.11 問題
{
  difficulty: '26.11',
  question: "パッチ26.11で新たに入手できるプライド関連エモートの価格は？",
  
  choices: ["ブルーエッセンスx1", "RPx100", "オレンジエッセンスx50", "無料配布のみ"],
  answer: 0,
  
},
{
  difficulty: '26.11',
  question: "パッチ26.11で復刻アイコンを購入できる期間は？",
  
  choices: ["パッチ26.11から26.13まで", "パッチ26.11のみ", "パッチ26.12から26.14まで", "常時販売"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11でクライアントのランク戦エリアから削除された機能は？",
  
  choices: ["自分のリーグ", "マスター以上ランキング", "フレンド比較", "ランク履歴"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11でミッドレーンのロールクエスト報酬はどう変更された？",
  
  choices: ["増加攻撃力と増加魔力が6%から8%に上昇", "経験値が減少", "移動速度が追加", "クエスト自体が削除"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11でヴォイドグラブから召喚されるヴォイドマイトの体力はどう変更された？",
  
  choices: ["前衛ミニオンの60%から100%に上昇", "前衛ミニオンの100%から60%に低下", "後衛ミニオンの100%に変更", "変更なし"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11でエアリー召喚の序盤シールド量はどう変更された？",
  image: "image/26.11/エアリー召喚.png",
  choices: ["30～100から20～100に低下", "20～100から30～100に上昇", "全レベルで100に固定", "削除された"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11でアフターショックの物理防御/魔法防御はどう変更された？",
  image: "image/26.11/アフターショック.png",
  choices: ["35から45に上昇", "45から35に低下", "スケーリングが完全削除", "クールダウンが短縮"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11でガーディアンのクールダウンはどう変更された？",
  image: "image/26.11/ガーディアン.png",
  choices: ["90～40秒から75～40秒に短縮", "75～40秒から90～40秒に増加", "常に40秒に固定", "削除された"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11で帝国の指令に追加された新自動効果は？",
  image: "image/26.11/帝国の指令.png",
  choices: ["制御と指令", "調整射撃と追撃", "献身と守護", "爆発と加速"],
  answer: 0,
},
    {
  difficulty: '26.11',
  question: "パッチ26.11で帝国の指令は、どのようなチャンピオン向けのアイテムになるようリワークされた？",
  image: "image/26.11/帝国の指令.png",
  choices: [
    "行動妨害でチームを支援したいユーティリティキャスター",
    "通常攻撃で継続ダメージを出すADC",
    "自身の回復量を高めるタンク",
    "クリティカルで火力を出すファイター"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11で帝国の指令の合計コストはどう変更された？",
  image: "image/26.11/帝国の指令.png",
  choices: [
    "2250ゴールドから2400ゴールドに増加",
    "2400ゴールドから2250ゴールドに低下",
    "2250ゴールドのまま変更なし",
    "2500ゴールドから2400ゴールドに低下"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11で帝国の指令の素材から外れたアイテムは？",
  image: "image/26.11/帝国の指令.png",
  choices: [
    "フィンディッシュの古書",
    "ブラスティング ワンド",
    "バンドルグラスの鏡",
    "増魔の書"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11で帝国の指令の新しい素材に追加されたアイテムは？",
  image: "image/26.11/帝国の指令.png",
  choices: [
    "ブラスティング ワンド",
    "フィンディッシュの古書",
    "キンドルジェム",
    "エーテル ウィスプ"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11で帝国の指令の魔力はどう変更された？",
  image: "image/26.11/帝国の指令.png",
  choices: [
    "+60から+65に増加",
    "+65から+60に低下",
    "+60のまま変更なし",
    "+60から+70に増加"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11で帝国の指令のスキルヘイストはどう変更された？",
  image: "image/26.11/帝国の指令.png",
  choices: [
    "+20から+15に低下",
    "+15から+20に増加",
    "+20のまま変更なし",
    "+20から+25に増加"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11で帝国の指令のマナ自動回復はどう変更された？",
  image: "image/26.11/帝国の指令.png",
  choices: [
    "125%から150%に増加",
    "150%から125%に低下",
    "100%から125%に増加",
    "125%のまま変更なし"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11で帝国の指令から削除された自動効果は？",
  image: "image/26.11/帝国の指令.png",
  choices: [
    "調整射撃",
    "制御",
    "指令",
    "弱点"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11で帝国の指令の『制御』は、どのスキルに15のスキルヘイストを与える？",
  image: "image/26.11/帝国の指令.png",
  choices: [
    "移動不能効果を持つスキル",
    "回復スキル",
    "シールドスキル",
    "通常攻撃強化スキル"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11で帝国の指令の『指令』は、敵チャンピオンに移動不能効果を与えると何秒間マークする？",
  image: "image/26.11/帝国の指令.png",
  choices: [
    "4秒間",
    "2秒間",
    "6秒間",
    "8秒間"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11で帝国の指令の『指令』によって付与される弱点の効果量は？",
  image: "image/26.11/帝国の指令.png",
  choices: [
    "6%",
    "4%",
    "8%",
    "10%"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でヘリアの残響の蓄積ダメージはどう変更された？",
  image: "image/26.11/ヘリアの残響.png",
  choices: ["35%から30%に低下", "30%から35%に上昇", "25%に低下", "変更なし"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11でソラリのロケットの物理/魔法防御はどう変更された？",
   image: "image/26.11/ソラリのロケット.png",
  choices: ["+25から+30に上昇", "+30から+25に低下", "+20から+25に上昇", "削除された"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11でブランドに行われた調整は？",
  image: "image/26.11/ブランド.png",
  choices: ["基本物理防御が27から24に低下", "基本物理防御が24から27に上昇", "移動速度が増加", "スキルヘイストが追加"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11でダイアナのモンスターへのダメージはどう変更された？",
  image: "image/26.11/ダイアナ.png",
  choices: ["230%から270%に上昇", "270%から230%に低下", "200%に低下", "変更なし"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11でエコーの固有スキルの敵ごとのクールダウンは？",
  images: [ "image/26.11/エコー.png",
"image/26.11/固有スキル - ゼロ・ドライブ.png",
             ],
  choices: ["5秒から4秒に短縮", "4秒から5秒に増加", "3秒に短縮", "削除された"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11でハイマーディンガーの砲台射程はどう変更された？",
  images: [ "image/26.11/ハイマーディンガー.png",
  "image/26.11/Q - H-28G革新砲.png",
           ],
  choices: ["530から550に増加", "550から530に低下", "600に増加", "変更なし"],
  answer: 0,
},
 {
  difficulty: '26.11',
  question: "パッチ26.11でハイマーディンガーに調整が入った主な理由は？",
  image: "image/26.11/ハイマーディンガー.png",
  choices: [
    "砲台とミニオンの射程の相互作用に問題があったため",
    "アルティメットのダメージが高すぎたため",
    "移動速度が高すぎたため",
    "ジャングル性能が高すぎたため"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でハイマーディンガーの砲台が困っていた相手は？",
  images: [  "image/26.11/ハイマーディンガー.png",
 "image/26.11/Q - H-28G革新砲.png",
      ],     
  choices: [
    "後衛ミニオン",
    "バロンナッシャー",
    "ドラゴン",
    "味方ミニオン"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でハイマーディンガーの砲台の視界範囲はどう変更された？",
 images: [  "image/26.11/ハイマーディンガー.png",
  "image/26.11/Q - H-28G革新砲.png",
           ],
  choices: [
    "585から650に増加",
    "650から585に低下",
    "530から550に増加",
    "550から530に低下"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でハイマーディンガーの砲台は、他に射程内の対象がいない場合どうなる？",
  images: ["image/26.11/ハイマーディンガー.png",
  "image/26.11/Q - H-28G革新砲.png",
             ],
  choices: [
    "ミニオンに対する射程が50増加する",
    "チャンピオンに対するダメージが50増加する",
    "攻撃速度が50%増加する",
    "砲台の体力が50増加する"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でハイマーディンガーのEの対象地点の視界確保時間はどう変更された？",
   images: [ "image/26.11/ハイマーディンガー.png",
  "image/26.11/E - CH-2超電磁グレネード.png",
            ],
  choices: [
    "1秒から1.25秒に増加",
    "1.25秒から1秒に低下",
    "1秒から2秒に増加",
    "変更なし"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でハイマーディンガーの革新砲が超電磁グレネードへの追撃に失敗する原因として報告されたものは？",
  images: ["image/26.11/ハイマーディンガー.png",
   "image/26.11/E - CH-2超電磁グレネード.png",
           ],
  choices: [
    "視界と砲台ロジックに関するわずかなタイミングのブレ",
    "マナ消費量の増加",
    "砲台の攻撃力低下",
    "移動速度の低下"
  ],
  answer: 0
} ,  
{
  difficulty: '26.11',
  question: "パッチ26.11でカサディンのQのクールダウンはどう変更された？",
  images: ["image/26.11/カサディン.png",
  "image/26.11/Q - ヴォイドスフィア.png",
           ],
  choices: ["全ランクで1秒短縮", "全ランクで1秒増加", "レベル1のみ短縮", "変更なし"],
  answer: 0,
},
    {
  difficulty: '26.11',
  question: "パッチ26.11でカサディンに調整が入った主な目的は？",
  image: "image/26.11/カサディン.png",
  choices: [
    "レーン戦のファーム面とダメージ交換を改善するため",
    "ジャングルクリア速度を上げるため",
    "アルティメットの射程を短くするため",
    "サポート運用を強化するため"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でカサディンの序盤戦はどのような状態だったと説明されている？",
  image: "image/26.11/カサディン.png",
  choices: [
    "これまで以上に厳しい状態だった",
    "非常に安定していた",
    "序盤から強すぎた",
    "ジャングル運用が強すぎた"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でカサディンのQ - ヴォイドスフィアの最大ランク時クールダウンはどう変更された？",
  images: [
    "image/26.11/カサディン.png",
  "image/26.11/Q - ヴォイドスフィア.png",
    ],
  choices: [
    "8秒から7秒に短縮",
    "7秒から8秒に増加",
    "8秒のまま変更なし",
    "10秒から9秒に短縮"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でカサディンのQ - ヴォイドスフィアのランク1クールダウンはどう変更された？",
  images: [
  "image/26.11/カサディン.png",
 "image/26.11/Q - ヴォイドスフィア.png",
    ],
  choices: [
    "10秒から9秒に短縮",
    "9秒から10秒に増加",
    "8秒から7秒に短縮",
    "変更なし"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でカサディンのW - ネザーブレードの基本魔法ダメージはどう変更された？",
    images: [
      "image/26.11/カサディン.png",
   "image/26.11/W - ネザーブレード.png",
        ],     
  choices: [
    "20から25に増加",
    "25から20に低下",
    "20から30に増加",
    "変更なし"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でカサディンのW - ネザーブレードの魔力反映率はどうなった？",
  images: [
    "image/26.11/カサディン.png",
  "image/26.11/W - ネザーブレード.png",
    ],
  choices: [
    "魔力の10%のまま変更なし",
    "魔力の10%から15%に増加",
    "魔力の15%から10%に低下",
    "魔力反映率が削除された"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でクインの固有スキルのモンスターへのダメージは？",
  images: [  
  "image/26.11/クイン.png",
   "image/26.11/Q - 暗闇の強襲.png",
    ],
  choices: ["50から75に上昇", "75から50に低下", "150から200に上昇", "変更なし"],
  answer: 0,
},
    {
  difficulty: '26.11',
  question: "パッチ26.11でクインに調整が入った主な目的は？",
  image: "image/26.11/クイン.png",
  choices: [
    "ジャングルクリア速度の競争力を高めるため",
    "対面性能を大きく強化するため",
    "バースト性能を下げるため",
    "サポート性能を上げるため"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でクインは、どの性能には手を加えない方針だった？",
  image: "image/26.11/クイン.png",
  choices: [
    "バースト性能や対面性能",
    "ジャングルクリア速度",
    "モンスターへのダメージ",
    "キャンプ処理能力"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でクインの固有スキル - 鷲匠のモンスターへのダメージはどう変更された？",
   images: [  
     "image/26.11/クイン.png",
 "image/26.11/固有スキル - 鷲匠.png",
     ],
  choices: [
    "50から75に上昇",
    "75から50に低下",
    "150%から200%に上昇",
    "変更なし"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でクインのQ - 暗闇の強襲のモンスターへのダメージはどう変更された？",
   images: [  
     "image/26.11/クイン.png",
"image/26.11/Q - 暗闇の強襲.png",
      ],
  choices: [
    "150%から200%に上昇",
    "200%から150%に低下",
    "50から75に上昇",
    "変更なし"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でクインの調整により、より頻繁に挑めるようになると説明されたものは？",
  image: "image/26.11/クイン.png",
  choices: [
    "敵陣深くでの危険な任務",
    "ミッドレーンでの集団戦",
    "味方への回復支援",
    "タワー下での防衛"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でティーモのレベルアップごとの物理防御はどう変更された？",
   image: "image/26.11/ティーモ.png",
  choices: ["4.95から4.5に低下", "4.5から4.95に上昇", "5.0に上昇", "変更なし"],
  answer: 0,
},
    {
  difficulty: '26.11',
  question: "パッチ26.11でティーモの調整対象として特に挙げられているビルドは？",
  image: "image/26.11/ティーモ.png",
  choices: [
    "シヴビルド",
    "タンクビルド",
    "サポートビルド",
    "ジャングルビルド"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でティーモの毒たっぷり吹き矢の攻撃力反映率を調整する目的は？",
  images: [
    "image/26.11/ティーモ.png",
   "image/26.11/E - 毒たっぷり吹き矢.png",
   ],
  choices: [
    "シヴビルドを適切な強さにし、他ビルドも選びやすくするため",
    "ティーモを完全にAD専用にするため",
    "ティーモのサポート性能を上げるため",
    "毒ダメージを削除するため"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でティーモのEの通常攻撃時効果の増加攻撃力反映率はどう変更された？",
  images: [
    "image/26.11/ティーモ.png",
  "image/26.11/E - 毒たっぷり吹き矢.png",
   ],
    choices: [
    "増加攻撃力の10%から5%に低下",
    "増加攻撃力の5%から10%に上昇",
    "増加攻撃力の20%から10%に低下",
    "変更なし"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でティーモのEの毒による継続ダメージごとの増加攻撃力反映率はどう変更された？",
    images: [
      "image/26.11/ティーモ.png",
   "image/26.11/E - 毒たっぷり吹き矢.png",
      ],
  choices: [
    "増加攻撃力の20%から10%に低下",
    "増加攻撃力の10%から20%に上昇",
    "増加攻撃力の10%から5%に低下",
    "変更なし"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11のティーモ調整で、他ビルドとして例に挙げられているものは？",
  image: "image/26.11/ティーモ.png",
  choices: [
    "ナッシャー、ライアンドリー、マリグナンス",
    "ソラリ、騎士の誓い、ジーク",
    "心の鋼、ワーモグ、サンファイア",
    "インフィニティ エッジ、コレクター、ドミニク"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でティーモのスケーリングによる耐久力を抑える理由として正しいものは？",
  image: "image/26.11/ティーモ.png",
  choices: [
    "レーンのハラス役という個性を維持しつつ、対面時に倒しやすくするため",
    "ジャングルクリアを速くするため",
    "サポートとして運用しやすくするため",
    "アルティメットのダメージを上げるため"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11で実験的ヘクスプレートの遠隔チャンピオン向け効果は？",
  image: "image/26.11/実験的ヘクスプレート.png",
  choices: ["攻撃速度35%、移動速度14%に低下", "攻撃速度50%、移動速度20%に上昇", "近接と同じまま", "効果が削除"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11で心の鋼の恒久体力へのダメージ変換率はどう変更された？",
  image: "image/26.11/心の鋼.png",
  choices: ["8%から10%に上昇", "10%から8%に低下", "12%に上昇", "変更なし"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11でスタティック シヴの攻撃力はどう変更された？",
  image: "image/26.11/スタティック シヴ.png",
  choices: ["40から45に上昇", "45から40に低下", "50に上昇", "変更なし"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: "パッチ26.11で死神の残り火のダメージタイプはどう変更された？",
  image: "image/26.11/死神の残り火.png",
  choices: ["魔法ダメージのみになった", "物理ダメージのみになった", "確定ダメージになった", "削除された"],
  answer: 0,
},
{
  difficulty: '26.11',
  question: 'パッチ26.11で「ドリーム メーカー」の青の夢の泡沫のダメージ軽減量はどう変更された？',
  image: "image/26.11/ドリーム メーカー.png",
  choices: [
    '75～255から50～194に低下',
    '50～194から75～255に上昇',
    '75～255から75～300に上昇',
    '50～194から40～160に低下'
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: 'パッチ26.11で「青の夢の泡沫」のダメージ軽減効果はどう変更された？',
  image: "image/26.11/ドリーム メーカー.png",
  choices: [
    '次の0でないダメージソース全体に持続適用される',
    '次に受ける通常攻撃1回のみに適用される',
    '味方全員に常時適用される',
    'スキルダメージには適用されなくなる'
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: 'パッチ26.11で「紫の夢の泡沫」のダメージはどう変更された？',
  image: "image/26.11/ドリーム メーカー.png",
  choices: [
    '50～170から40～160に低下',
    '40～160から50～170に上昇',
    '75～255から50～194に低下',
    '35%から30%に低下'
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: 'パッチ26.11で「紫の夢の泡沫」はどのように発動するようになった？',
  image: "image/26.11/ドリーム メーカー.png",
  choices: [
    '次のダメージを与える通常攻撃またはスキルで発動する',
    '通常攻撃時のみ発動する',
    '回復またはシールド使用時のみ発動する',
    'アルティメットスキル使用時のみ発動する'
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: 'パッチ26.11の「ドリーム メーカー」変更で、範囲効果にはどのような調整が入った？',
  image: "image/26.11/ドリーム メーカー.png",
  choices: [
    '範囲効果では効果が低下する',
    '範囲効果では効果が2倍になる',
    '範囲効果では発動しなくなる',
    '範囲効果ではクールダウンがなくなる'
  ],
  answer: 0
} ,
{
  difficulty: '26.11',
  question: 'パッチ26.11で「ムーンストーンの再生」は何が二重に効果を発揮しなくなった？',
  image: "image/26.11/ムーンストーンの再生.png",
  choices: [
    '回復/シールド効果と重傷/盾崩し',
    '攻撃速度と移動速度',
    '物理防御と魔法防御',
    'スキルヘイストとマナ自動回復'
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: 'パッチ26.11の「ムーンストーンの再生」変更は基本的にどのような調整？',
  image: "image/26.11/ムーンストーンの再生.png",
  choices: [
    '基本的には弱体化',
    '完全な強化',
    '性能変更なし',
    'アイテム削除'
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: 'パッチ26.11で、相手が重傷/盾崩しを持っている場合の「ムーンストーンの再生」はどうなることがある？',
  image: "image/26.11/ムーンストーンの再生.png",
  choices: [
    '結果的に強化となることがある',
    '完全に無効化される',
    '価格が下がる',
    '回復効果が消える'
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: 'パッチ26.10では、回復/シールド効果50%で150回復した場合、別の味方への連鎖回復量はいくつだった？',
  image: "image/26.11/ムーンストーンの再生.png",
  choices: [
    '67.5',
    '45',
    '30',
    '150'
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: 'パッチ26.11以降、回復/シールド効果50%で150回復した場合、別の味方への連鎖回復量はいくつになる？',
  image: "image/26.11/ムーンストーンの再生.png",
  choices: [
    '45',
    '67.5',
    '150',
    '30'
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: 'パッチ26.11で「ムーンストーンの再生」の連鎖効果はどの味方を優先するようになった？',
  image: "image/26.11/ムーンストーンの再生.png",
  choices: [
    '体力の低い味方',
    '最も近い味方',
    '攻撃力が高い味方',
    '自分自身'
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11で騎士の誓いは、どの方向で調整された？",
  image: "image/26.11/騎士の誓い.png",
  choices: [
    "自動効果を強化する方向",
    "ゴールドコストを下げる方向",
    "ステータスを大幅に上げる方向",
    "アイテム効果を削除する方向"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11で騎士の誓いのダメージ転送はどう変更された？",
  image: "image/26.11/騎士の誓い.png",
  choices: [
    "12%から14%に上昇",
    "14%から12%に低下",
    "10%から12%に上昇",
    "変更なし"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11で騎士の誓いの体力回復量はどう変更された？",
  image: "image/26.11/騎士の誓い.png",
  choices: [
    "10%から12%に上昇",
    "12%から10%に低下",
    "8%から10%に上昇",
    "変更なし"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でジーク コンバージェンスは、どのようなチャンピオン向けにQoL改善された？",
  image: "image/26.11/ジーク コンバージェンス.png",
  choices: [
    "遠距離からアルティメットスキルを使用するチャンピオン",
    "通常攻撃を主体に戦うマークスマン",
    "回復やシールドを中心に使うエンチャンター",
    "ジャングルクリアを高速化したいチャンピオン"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でジーク コンバージェンスの氷炎のテンペストは、いつ準備状態になる？",
  image: "image/26.11/ジーク コンバージェンス.png",
  choices: [
    "アルティメット発動時から5秒間",
    "通常攻撃後から3秒間",
    "敵チャンピオンに移動不能効果を与えた時から4秒間",
    "味方にシールドを付与した時から5秒間"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でジーク コンバージェンスの氷炎のテンペストは、どの条件で発動するようになった？",
  image: "image/26.11/ジーク コンバージェンス.png",
  choices: [
    "準備中に敵チャンピオンが範囲内に入るか、5秒後に発動",
    "アルティメットを使った瞬間に必ず即発動",
    "味方チャンピオンが回復された時に発動",
    "敵ミニオンを倒した時に発動"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11以前のジーク コンバージェンスの氷炎のテンペストは、どのタイミングで発動していた？",
  image: "image/26.11/ジーク コンバージェンス.png",
  choices: [
    "アルティメット発動時に発動",
    "通常攻撃時に発動",
    "敵チャンピオンにキルされた時に発動",
    "味方にシールドを付与した時に発動"
  ],
  answer: 0
},
 {
  difficulty: '26.11',
  question: "パッチ26.11でスモルダーは、主にどのビルドへ戻す方向で調整された？",
  image: "image/26.11/スモルダー.png",
  choices: [
    "クリティカルビルド",
    "ブルーザー型ビルド",
    "タンクビルド",
    "サポートビルド"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でスモルダーがクリティカルビルド寄りに調整された理由として正しいものは？",
  image: "image/26.11/スモルダー.png",
  choices: [
    "耐久力が低く、敵のスキルコンボで倒されやすいため",
    "タンクとして強すぎたため",
    "サポート運用が強すぎたため",
    "ジャングルクリアが遅すぎたため"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でスモルダーのQの追加魔法ダメージの基本倍率はどう変更された？",
  images: [
       "image/26.11/スモルダー.png",
 "image/26.11/固有スキル - 駆けだしドラゴン.png",
       ],
  choices: [
    "固有スキルのスタックの40%から25%に低下",
    "固有スキルのスタックの25%から40%に上昇",
    "固有スキルのスタックの12%から8%に低下",
    "変更なし"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でスモルダーのQの追加魔法ダメージは、どの要素の影響をより受けるようになった？",
  image: "image/26.11/固有スキル - 駆けだしドラゴン.png",
  choices: [
    "クリティカル率と増加クリティカルダメージ",
    "物理防御と魔法防御",
    "最大体力とマナ",
    "移動速度と攻撃速度"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でスモルダーのEの追加魔法ダメージの基本倍率はどう変更された？",
images: [
     "image/26.11/スモルダー.png", 
"image/26.11/固有スキル - 駆けだしドラゴン.png",
      ],
  choices: [
    "固有スキルのスタックの12%から8%に低下",
    "固有スキルのスタックの8%から12%に上昇",
    "固有スキルのスタックの40%から25%に低下",
    "変更なし"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でスモルダーのEの追加魔法ダメージは、何によってスケーリングするようになった？",
images: [
"image/26.11/スモルダー.png",
"image/26.11/固有スキル - 駆けだしドラゴン.png",
      ],
  choices: [
    "クリティカル率と増加クリティカルダメージ",
    "スキルヘイスト",
    "最大マナ",
    "レベルごとの物理防御"
  ],
  answer: 0
} ,
  {
  difficulty: '26.11',
  question: "パッチ26.11でシン・ジャオの魔力ビルドに対して調整が入った主な理由は？",
  image: "image/26.11/シン・ジャオ.png",
  choices: [
    "膨大な回復量が予想を上回っていたため",
    "通常攻撃の射程が長すぎたため",
    "ジャングルクリアが遅すぎたため",
    "アルティメットの範囲が広すぎたため"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でシン・ジャオは、回復量を抑える代わりに何で補填された？",
  image: "image/26.11/シン・ジャオ.png",
  choices: [
    "継続火力",
    "移動速度",
    "物理防御",
    "シールド量"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でシン・ジャオの固有スキル - 不退転の追加ダメージに新たに加わった反映率は？",
  images: [  
  "image/26.11/シン・ジャオ.png",
  "image/26.11/固有スキル - 不退転.png",
    ],
  choices: [
    "魔力の5 / 10 / 15 / 20%",
    "魔力の45 / 55 / 80%",
    "増加体力の5 / 10 / 15 / 20%",
    "攻撃速度の5 / 10 / 15 / 20%"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でシン・ジャオの固有スキル - 不退転の追加ダメージの増加攻撃力反映率はどうなった？",
images: [
"image/26.11/シン・ジャオ.png",
"image/26.11/固有スキル - 不退転.png",
         ],
  choices: [
    "15 / 30 / 45 / 60%のまま維持された",
    "15 / 30 / 45 / 60%から10 / 20 / 30 / 40%に低下した",
    "魔力反映率に完全に置き換わった",
    "削除された"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でシン・ジャオの固有スキル - 不退転の体力回復量の魔力反映率はどう変更された？",
images: [
"image/26.11/シン・ジャオ.png",
"image/26.11/固有スキル - 不退転.png",
        ],
  choices: [
    "50 / 65 / 90%から45 / 55 / 80%に低下",
    "45 / 55 / 80%から50 / 65 / 90%に上昇",
    "5 / 10 / 15 / 20%に変更",
    "魔力反映率が削除された"
  ],
  answer: 0
},
{
  difficulty: '26.11',
  question: "パッチ26.11でシン・ジャオの固有スキル - 不退転の体力回復量の最大体力反映率はどうなった？",
  images: [ 
  "image/26.11/シン・ジャオ.png",
  "image/26.11/固有スキル - 不退転.png",
      ],
  choices: [
    "最大体力の3 / 4 / 5%のまま維持された",
    "最大体力の3 / 4 / 5%から2 / 3 / 4%に低下した",
    "最大体力反映が削除された",
    "最大体力の5 / 10 / 15%に上昇した"
  ],
  answer: 0
},
  ];
};
